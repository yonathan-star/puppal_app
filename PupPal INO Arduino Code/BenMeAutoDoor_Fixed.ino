...
// Timekeeping (minutes since midnight). Set by app via Bluetooth command SETTIME:<minutes>
int g_currentMinutesSinceMidnight = -1; // -1 means unknown
...
int nowMinutes() {
  return g_currentMinutesSinceMidnight;
}
...
void handleAsciiCommand(const String &cmd) {
  if (cmd.startsWith("SETWINS:")) {
    ...
    // SETMETA:UID:grams:density - Forward to other devices (AutoDoor doesn't use this data)
    ...
    int p = cmd.indexOf(':');
    if (p > 0) {
      int minutes = cmd.substring(p+1).toInt();
      if (minutes >= 0 && minutes < 24*60) {
        g_currentMinutesSinceMidnight = minutes;
        Serial.print("⏰ Time set to minutes since midnight: ");
        Serial.println(g_currentMinutesSinceMidnight);
      }
    }
  }
}
