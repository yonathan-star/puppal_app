...
class LocalFeedingAI {
  ...
  static double _baseFactor(FeedingInput i) {
    if (i.species == Species.dog) {
      ...
      switch (i.ageStage) {
        case AgeStage.puppyKitten:
          // Increase cat kitten energy to align with observed needs
          // Typical: kittens need ~2.5x RER baseline
          double base = i.neutered ? 2.4 : 2.6;
          // Light scaling by weight to avoid underfeeding heavier kittens
          final weightLb = i.weightKg * 2.2046226218;
          if (weightLb >= 6.0 && weightLb <= 10.0) base += 0.1; // +4%
          if (weightLb > 10.0) base += 0.2; // +8%
          return base;
        case AgeStage.adult:
