...
import 'package:puppal_app1/services/food_database.dart';
import 'package:puppal_app1/services/ai_density_service.dart';
import 'package:puppal_app1/services/arduino_service.dart';
import 'package:puppal_app1/ai/local_feeding_ai.dart';
              ...
              onPressed: () async {
                ...
                if (ArduinoService.isConnected) {
                  try {
                    ...
                    await SyncQueueService.queueForSync(updatedProfile);
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            '⚠️ Saved locally. Will retry sync when device reconnects.',
                            textAlign: TextAlign.center,
                          ),
                          duration: const Duration(seconds: 3),
                          behavior: SnackBarBehavior.floating,
                  ...
                  // Queue for sync when device comes online
                  await SyncQueueService.queueForSync(updatedProfile);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          '💾 Saved locally. Will sync when device connects.',
                          textAlign: TextAlign.center,
                        ),
                        duration: Duration(seconds: 3),
                        behavior: SnackBarBehavior.floating,
