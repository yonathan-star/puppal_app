...
import 'package:puppal_app1/shared/styled_button.dart';
...
import 'package:puppal_app1/services/sync_queue_service.dart';
...
class _BleManagerScreenState extends State<BleManagerScreen> {
  ...
  Future<void> _connect(BluetoothDevice device) async {
    ...
    try {
      ...
      // Set connection and process sync queue
    ...
    if (uidMatch != null) {
      ...
      if (_pendingAddType == 'dog' || _pendingAddType == 'cat') {
        final newProfile = PetProfile(
          uidHex: hex,
          type: _pendingAddType!,
          name: null,
        );
        await ProfileStorage.upsert(newProfile);

        // Queue for sync to other devices
        await SyncQueueService.queueForSync(newProfile);

        _appendLog(
          '📝 New $_pendingAddType profile created and queued for sync',
        );
      }
      return;
    }
  ...
  void _handleSimulatorCommand(String cmd) async {
    if (cmd == 'GETUIDLIST') {
    ...
    if (cmd == '0' || cmd == '1') {
      ...
      if (!_uidList.contains(uid)) {
        ...
        final newProfile = PetProfile(uidHex: uid, type: t, name: null);
        await ProfileStorage.upsert(newProfile);
          ...
          Row(
            children: [
              Expanded(
                child: StyledButton(
                  onPressed: () async {
                    // Let user choose which UID to delete instead of removing last
                    if (_uidList.isEmpty) {
                      _appendLog('No UID to remove');
                      return;
                    }
                    final chosen = await showDialog<String>(
                      context: context,
                      builder: (_) => AlertDialog(
                        title: const Text('Select UID to remove'),
                        content: SizedBox(
                          width: double.maxFinite,
                          child: FutureBuilder<List<PetProfile>>(
                            future: ProfileStorage.load(),
                    ...
                    if (chosen != null) {
                      await _confirmAndDeleteUid(chosen);
                    }
                  },
                  child: Text(
                    'Remove Pet',
                      ...
                      builder: (context, snapshot) {
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            final profile = profiles.firstWhere(
                              (p) =>
                                  p.uidHex.toUpperCase() == uid.toUpperCase(),
                              orElse: () =>
                                  PetProfile(uidHex: uid, type: 'unknown'),
                            );
                            final displayName =
                                (profile.name != null &&
                                    profile.name!.isNotEmpty)
  ...
  Future<void> _confirmAndDeleteUid(String uidHex) async {
    ...
    final profiles = await ProfileStorage.load();
    final profile = profiles.firstWhere(
      (p) => p.uidHex.toUpperCase() == uidHex.toUpperCase(),
      orElse: () => PetProfile(uidHex: uidHex, type: 'unknown'),
    );
